    ...
    <>
      {open && (
        <div className="fixed inset-0 z-40 flex items-center justify-center px-2">
          ...
          <div
            ref={modalRef}
            className="relative z-50 bg-white w-[90%] sm:max-w-md max-h-[90vh] overflow-y-auto p-4 sm:p-6 rounded-2xl shadow-2xl border border-gray-200"
          >
            {/* Header */}
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-3">
                  {typeOptions.map(({ type: ct, icon, label }) => (
                    <button
                      key={ct}
                      onClick={() => setType(ct)}
                      className={`flex flex-col items-center justify-center border rounded-lg p-2 sm:p-3 text-xs font-medium hover:bg-gray-100 transition ${
                        type === ct
